...
<html lang="{{ app()->getLocale() }}">
    <head>
        ...
        <title>Patrol Report | {{ config('app.name') }}</title>
    ...
    <body style="background-color: #f6f7fb;">
        <div class="container-fluid" style="height: 100vh;">
            ...
            <div class="row" style="margin-bottom: 20px;">
                <div class="col-12">
                    {!! Form::model($param, ['route' => ['reports.clients.patrols.special', $client->slug()], 'class' => 'form-inline']) !!}
                            ...
                            <table id="myTable4" class="display-1 table table-condensed table-hover table-striped responsive" width="100%">
                                <thead>
                                    <tr class="text-center">
                                        <th><strong>DATE/TIME LOGGED</strong></th>
                                ...
                                <tbody>
                                    @foreach ($visits as $visit)
                                        @foreach (App\AmdErsVisitDetail::where('ers_visit_id', $visit->id)->get() as $detail)
                                    <tr>
                                        <td>{{ $visit->created_at }}</td>
                                        <td>HAL/PAT/{{ sprintf('%05d', $visit->id) }}</td>
                                        <td>{{ App\AmdErsLocation::find($visit->ers_location_id)->name }}</td>
                                        <td>{{ $visit->entry_time }}</td>
                                        <td>{{ $visit->exit_time }}</td>
                                        <td>{{ $detail->description }}</td>
                                        <td>{{ $detail->option }}</td>
                                        <td>
                                            @if ($visit->user_id) {{ App\AmdUser::find($visit->user_id)->full_name }} @endif
                                        </td>
                                    </tr>
                                        @endforeach
                                    @endforeach
